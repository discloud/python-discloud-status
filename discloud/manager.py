...
class RequestManager:
    ...
    async def fetch_bot(self, bot_id: int): # todo: typehint
        await self.request(Route("POST", "/bot/{bot_id}", bot_id=bot_id))

    async def fetch_user(self): # todo: typehint
        await self.request(Route("POST", "/user"))

    async def restart(self, bot_id: int): # todo: typehint
        await self.request(Route("POST", "/bot/{bot_id}/restart", bot_id=bot_id))
